                ...
                >
                  ...
                  {p.thumb && (
                    <button className="thumb-nav-btn thumb-wa-btn" title="Share via WhatsApp"
                      onClick={() => {
                        setMobileNavIdx(null);
                        const year = p.dt ? new Date(p.dt).getFullYear() : null;
                        const link = config.webAppUrl + '/?photo=' + p.hash + (year ? '&year=' + year : '');
                        setWaShare({ photo: p, caption: link, sending: false });
                      }}
                      onTouchEnd={e => {
                        e.preventDefault(); e.stopPropagation(); setMobileNavIdx(null);
                        const year = p.dt ? new Date(p.dt).getFullYear() : null;
                        const link = config.webAppUrl + '/?photo=' + p.hash + (year ? '&year=' + year : '');
                        setWaShare({ photo: p, caption: link, sending: false });
                      }}
